...
export const fetchProductAction = (token) => async (dispatch) => {
  await apiClient
    .fetchProducts(token)
    .then((res) => {
      dispatch({ type: ProductTypes.FETCH_PRODUCT, payload: res.data });
    })
    ...
    await apiClient
      ...
      .then((res) => {
        if (res.data.status == 200) {
          dispatch({ type: ProductTypes.STORE_PRODUCT, payload: res.data });
          Swal.fire({
            icon: "success",
            title: "Success",
